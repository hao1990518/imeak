...
class Slide extends Base
{
    ...
    public function index()
    {
        if(Request::instance()->isAjax()) {
            ...
            //获取幻灯片结果数据
            $data['data'] = $SlideModel->getSlideList($condition, $start, $number, $order);
            //总数
            $total = $SlideModel->getSlideCount($condition);
            ...
            if ($data['data']) {
                foreach ($data['data'] as $k => $v) {
                    ...
                    $data['aaData'][$k][] = "
                        <a href='" . url('Slide/edit', ['id' => $v['id']]) . "' alt='编辑' title='编辑' style='color:#676A6C'><i class='fa fa-edit'></i></a>
    ...
    public function add()
    {
        ...
        $SlideModel = new SlideModel();
        $category_list = $SlideModel->getCategorys([]);
    ...
    public function edit($id)
    {
        if(Request::instance()->isPost()){
            ...
            $id = $data['id'];
            unset($data['id']);
            if(SlideModel::update($data,['id'=>$id])){
                $this->success('编辑成功');
            }else{
                $this->error('编辑失败');
            }
    ...
    public function category()
    {
        if(Request::instance()->isAjax()) {
            ...
            $start = (int)@$tableInfo['start'] > 0 ? $tableInfo['start'] : 0;
            //获取管理员结果数据
            $data['data'] = $SlideModel->getCategoryList($condition, $start, $number, $order);
            //总数
            $total = $SlideModel->getCategoryCount($condition);
    ...
    public function addCategory()
    {
        if(Request::instance()->isPost()){
            ...
            if(empty($data['name'])){
                $this->error('名称不能为空');
            }elseif($SlideModel->getCategory(['name'=>$data['name']])){
                $this->error('名称已存在');
            }else{
                $data['uid'] = Session::get(config('user_auth_key'));
                if($SlideModel->addCategory($data)){
    ...
    public function editCategory($id)
    {
        ...
        if(Request::instance()->isPost()){
            ...
            if(empty($data['name'])){
                $this->error('名称不能为空');
            }elseif($SlideModel->getCategory(['name'=>$data['name'],'id'=>array('neq',$data['id'])])){
                $this->error('名称已存在');
            }else{
                $id = $data['id'];
                unset($data['id']);
                if($SlideModel->updateCategory($data,['id'=>$id])){
        ...
        $Category = $SlideModel->getCategory(['id'=>$id]);
...
//                like '%,1,%' or like '1,%' or ='1' or like '%,1'
                if($SlideModel::get(['category_id'=> [['like','%,'.$id.',%'], ['like',$id.',%'],['like','%,'.$id], ['eq',$id], 'or']])){
                    $this->error('该分类下有幻灯片，不能删除');
                }else{
                    if($SlideModel->deleteCategory(['id'=>$id])){
                        $this->success('删除成功');
                    }else{
                        $this->error('删除失败');
                    }
